...
class TransactionsSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, obj):
        ...
        ret["commodity"] = CommoditySerializer(
            Commodity.objects.get(id=ret["commodity"])
        ).data
...
class InternalTransactionsSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, obj):
        ...
        ret["source"] = SourcesSerializer(Sources.objects.get(id=ret["source"])).data
        ret["destination"] = SourcesSerializer(
            Sources.objects.get(id=ret["destination"])
        ).data
        obj.date = timezone.localtime(obj.date)
        ret["date"] = obj.date.strftime("%Y-%m-%d %H:%M")
        return ret
...
class SourcesSerializer(serializers.ModelSerializer):
    class Meta:
        model = Sources
        fields = ["id", "user", "name", "default", "is_active"]
        extra_kwargs = {"user": {"write_only": True}}
